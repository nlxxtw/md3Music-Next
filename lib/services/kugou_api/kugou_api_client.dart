...
class KugouApiClient {
  ...
  /// 获取社区音效列表（/get/model → ocean/v6/sound/list）。
  ///
  /// [sort] 排序方式：2=默认，3=最热，4=最新；[pagesize] 每页数量，默认 30。
  /// 响应 `{ status: 1, data: [ {id, name, classify, tag_name, sound, intro, ...} ] }`，
  /// classify=2 为官方蝰蛇音效，classify=3 为社区音效。
  Future<Map<String, dynamic>?> getSoundModel({
    int sort = 2,
    int page = 1,
    int pagesize = 30,
  }) async {
  ...
  Future<Map<String, dynamic>?> importPlaylistByUrl(String url) async {
    return await _post(
      KugouEndpoints.importPlaylist,
      data: {'operation': 'add_task', 'task_type': 0, 'url': url},
      queryParameters: _importTs(),
      noCache: true,
    );
  }

  /// 截图导入第一步：逐张上传截图（同一次导入使用相同 [taskSn]）。
  /// [imgBase64] 允许带 `data:image/xxx;base64,` 前缀，服务端会剥离。
  Future<Map<String, dynamic>?> importPlaylistSubmitImg({
    required String taskSn,
  ...
  /// 截图导入第二步：截图上传完毕后创建云端导入任务。
  /// [taskSn] 需与 submit_img 一致；[listid] 目标歌单 ID（0 表示新建），
  /// [listName] 新建歌单名称。
  Future<Map<String, dynamic>?> importPlaylistByImages({
    required String taskSn,
    required String listid,
    String? listName,
  }) async {
  ...
  /// 查询导入任务数量：operation=task_count，[classify] 默认 1。
  Future<Map<String, dynamic>?> importPlaylistTaskCount({
    int classify = 1,
  }) async {
    return await _post(
      KugouEndpoints.importPlaylist,
      data: {'operation': 'task_count', 'classify': classify},
      queryParameters: _importTs(),
      noCache: true,
    );
  }

  /// 查询导入任务状态：operation=query_task_status、ids=[任务 ID]。
  /// 状态 3=成功、>=10=失败、其余=处理中。
  Future<Map<String, dynamic>?> importPlaylistQueryTaskStatus(
    List<int> ids,
  ) async {
    return await _post(
      KugouEndpoints.importPlaylist,
      data: {'operation': 'query_task_status', 'ids': ids},
      queryParameters: _importTs(),
      noCache: true,
    );
  }

  /// 查询导入结果：operation=query_task。
  /// 注意 [listid] 是导入后的歌单 ID，不是任务 ID。
  Future<Map<String, dynamic>?> importPlaylistQueryTask({
    required String listid,
    int page = 1,
    int pagesize = 30,
